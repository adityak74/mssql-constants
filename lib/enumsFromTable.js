module.exports = function () {
  return function readTable(defWrapper, cb) {
    ...
    var enumKeys = tableRows.map(function(row) {
      return keyTransform(row[keyColumn]);
    });
    ...
    defWrapper.enums = def.valueColumns.map(function(value) {
      ...
      enumKeys.forEach(function(key, index) {
        if(!enumerable[key]) enumerable[key] = enumValues[index];
        else {
          //Key already exists, so we append a number to identify the duplicate
          var duplicateIndex = 2;
          while(enumerable[key + enumDuplicateIndex]) enumDuplicateIndex += 1;
	  enumerable[key + enumDuplicateIndex] = enumValues[index]
        }
      });
      return enumerable;
    });
